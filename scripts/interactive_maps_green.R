...
create_interactive_map <- function(purchases_df) {
  ...
  color_palette <- c('#dff7ea', '#acdcd1', '#79c1b7', '#46a69e', '#408590', '#3a6381')
  
  color_income <- colorFactor(color_palette, zip_data$inc_brks, na.color = '#344273')
  color_race <- colorFactor(color_palette, zip_data$race_brks, na.color = '#344273')
  ...
  for (metro in metros) {
    ...
    color_gradient <- colorRamp(color_palette, interpolate = 'spline')
    data[[metro]]$color_pop <- colorNumeric(color_gradient, cbsa_zip_shps$pop_total, na.color = '#344273')
...
saveWidget(create_interactive_map(uiuc), file.path(normalizePath(outputs_dir), 'map_segment_green.html'), background = 'transparent', selfcontained = T)
saveWidget(create_interactive_map(poc), file.path(normalizePath(outputs_dir), 'map_poc_green.html'), background = 'transparent', selfcontained = T)
