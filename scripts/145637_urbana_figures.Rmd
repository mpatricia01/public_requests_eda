...
IL_orders <- IL_orders %>% mutate(year= format(date_start, format = "%Y"))
IL_orders <- IL_orders %>% mutate(order_index = seq(n()))

# out-of-state orders
OOS_orders <- OOS_orders %>% mutate(year= format(date_start, format = "%Y"))
OOS_orders <- OOS_orders %>% mutate(order_index = seq(n()))
...
### Total number of orders, students, & spending

2017-2020 = 51 orders; 121,110 students; $53,840.30:

- 2017 = 6 orders; 22311 students; $9,593.73
- 2018 = 18 orders; 45173 students; $19,424.39
- 2019 = 21 orders; 43295 students; $19,966.61	
- 2020 = 6 orders; 10331 students; $4,855.57


```{r, echo=FALSE}
IL_orders %>%
   ggplot(aes(x=order_num, y=num_students, fill= factor(year))) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
  

IL_orders %>%
   ggplot(aes(x=order_num, y=order_cost, fill= factor(year))) +
  geom_bar(stat="identity") +  
...
```{r include=FALSE}
#descriptive
IL_orders %>% summarise(total_students = sum(num_students))
IL_orders %>%   summarise(total_costs = sum(order_cost))

IL_orders %>% group_by(year) %>% summarise(total_orders = n())
IL_orders %>% group_by(year) %>% summarise(total_students = sum(num_students))
IL_orders %>% group_by(year) %>% summarise(total_costs = sum(order_cost))
...
## All years, out-of-state

### Total number of orders, students, & spending

2018-2020 = 22 orders; 142,744 students; $641,18.54

- 2018 = 9 orders; 55879 students; $24,027.97
- 2019 = 11 orders; 61858 students; $28,337.28	
- 2020 = 2 orders; 25007 students; $117,53.29
...
```{r include=FALSE}
#descriptive
OOS_orders %>% summarise(total_students = sum(num_students))
OOS_orders %>%   summarise(total_costs = sum(order_cost))

OOS_orders %>% group_by(year) %>% summarise(total_orders = n())
OOS_orders %>% group_by(year) %>% summarise(total_students = sum(num_students))
OOS_orders %>% group_by(year) %>% summarise(total_costs = sum(order_cost))
...
```{r include=FALSE}
OOS_orders <- OOS_orders %>% mutate(order_cat = ifelse(order_num %in% c('500590', '567376', '483751'), "MSA", "ENG"))

OOS_orders <- OOS_orders %>% mutate(order_cat = as.factor(ifelse(order_num %in% c('483724', '470283', '371629', '456737', '386335', '403340'), "Non-ENG", order_cat)))

OOS_orders %>% count(order_cat)

OOS_orders_grouped <- OOS_orders %>% select(order_cat, order_cost, num_students) %>%
    group_by(order_cat) %>% 
...
OOS_orders_grouped %>%
   ggplot(aes(x=order_cat, y=num_students)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
...
### OOS Order Category: Non-ENG orders 
...
### OOS Order Category: International
...
## Communities: Out-of-State, zip codes purchased from vs. not purchased from

All "non-purchased" zipcodes included below are only those in states where at least one student was purchased within at least one zipcode

  - Binary version of purchased_zip is problematic because of variation in number of students purchased by zip

```{r include=FALSE}
lists_df_sat_US <- lists_df_sat %>% filter(Country=="United States")

lists_df_sat_US <- lists_df_sat_US %>% filter(State!="IL")


lists_df_sat_US <- lists_df_sat_US %>% mutate(zip = substr(ZipCode, 1, 5))

acs_race_zipcode <- read_csv("../data/acs_race_zipcode.csv")
zip_to_state <- read_csv("../data/zip_to_state.csv")
...
statelist <- lists_df_sat_US %>% group_by(State) %>% count()
ziplist <- lists_df_sat_US %>% group_by(zip) %>% count()

acs_race_zipcodev2$purchased_zip <- ifelse(acs_race_zipcodev2$zip_code %in% ziplist$zip,1,0)
acs_race_zipcodev2$purchased_state <- ifelse(acs_race_zipcodev2$state_code %in% statelist$State,1,0)


acs_race_zipcodev2 %>% count(purchased_zip)
acs_race_zipcodev2 %>% count(purchased_state)

acs_race_zipcode <- acs_race_zipcodev2 %>% filter(purchased_state==1)

acs_race_zipcode <- acs_race_zipcode %>% mutate(median_household_income = ifelse(median_household_income==-666666666, NA, median_household_income))

acs_race_zipcode_grouped <- acs_race_zipcode %>% select(purchased_zip, median_household_income, pop_white_15_19_pct, pop_black_15_19_pct, pop_hispanic_15_19_pct, pop_asian_15_19_pct) %>%
    group_by(purchased_zip) %>% 
...
acs_race_zipcode_grouped %>%
   ggplot(aes(x=as.factor(purchased_zip), y=median_household_income)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
  
acs_race_zipcode_grouped %>%
   ggplot(aes(x=as.factor(purchased_zip), y=pop_white_15_19_pct)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 

acs_race_zipcode_grouped %>%
   ggplot(aes(x=as.factor(purchased_zip), y=pop_asian_15_19_pct)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 

acs_race_zipcode_grouped %>%
   ggplot(aes(x=as.factor(purchased_zip), y=pop_hispanic_15_19_pct)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 


acs_race_zipcode_grouped %>%
   ggplot(aes(x=as.factor(purchased_zip), y=pop_black_15_19_pct)) +
  geom_bar(stat="identity") +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```


## Out-of-State, zip codes charactertistics based on # of names purchased

Large variation in the number of students purchased by zip codes (where at least one student purchased) 
  - Across all purchased zips: mean=40, median=10, sd=76
...
acs_race_zipcodev2 <- acs_race_zipcodev2 %>%
  mutate(purchased_num = ifelse(purchased_zip==0, 0, purchased_num))

lbls <- c('zero','1-10','11-40','41-100','101-500', '500+')

acs_race_zipcodev2 <- acs_race_zipcodev2 %>% mutate(purchased_num_cat =cut(purchased_num, breaks=c(-Inf, 0, 10, 40, 100, 500, +Inf),labels = lbls)) 


acs_race_zipcode_grouped <- acs_race_zipcodev2 %>% select(median_household_income, pop_white_15_19_pct, pop_black_15_19_pct, pop_hispanic_15_19_pct, pop_asian_15_19_pct, purchased_num_cat) %>%
...
```{r, echo=FALSE}
# Zip Codes purchased by State
#acs_race_zipcodev2 %>% group_by(state_code) %>% count()


#Across all purchased zips
acs_race_zipcodev2 %>% filter(purchased_zip==1) %>%  select(purchased_num) %>%
      summarise(across(
        .fns = list(mean = mean, sd = sd, med=median, min = min, max =max), na.rm = TRUE )) 


# Average num of students purchased within zips by state
acs_race_zipcodev2 %>% filter(purchased_zip==1) %>% group_by(state_code)  %>% select(purchased_num) %>%
      summarise(across(
        .fns = list(mean = mean, sd = sd, med=median, min = min, max =max), na.rm = TRUE )) 


# Tabulate categorical purchased var within purchased states
acs_race_zipcodev2 %>% filter(purchased_state==1) %>% count(purchased_num_cat)
...
## Schools: Descriptives across U.S. (filtered by purchased states)

## Schools: Zoom into  Metros (filtered by purchased states)
