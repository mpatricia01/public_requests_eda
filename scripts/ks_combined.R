...
#Show results via map too?
      ...
      #COL 1: All Students (across all years & univs) by race/ethnicity, avg income
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% count()
       
       #race
       lists_orders_zip_hs_df %>% 
              summarize(pctwhite = mean(stu_white_common, na.rm=TRUE),
                        pctasian = mean(stu_asian_common, na.rm=TRUE),
                        pctblack = mean(stu_black_common, na.rm=TRUE),
                        pcthisp = mean(stu_is_hisp_common, na.rm=TRUE))
       
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       
       #in comparison to 2019 US 15-19 year olds: ~22M (53% White; 15% Black; 24% Hispanic; 6% Asian; 1% Native)
      # look at racial composition of the states where universities are located
       
       #income 
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% 
         summarise (mean_inc = mean(zip_median_household_income, na.rm=T)) 
       
       #in comparison to 2019 US median household income $68,703
       # look at comparison of median household income for states where universities are located
       
       
       #COL 2: Out-of-State/In-State by race/ethnicity, avg income
       
       #construct outofstate var
       lists_orders_zip_hs_df <- lists_orders_zip_hs_df %>%
         mutate(stu_internat = ifelse(stu_country=="united states", 0, 1),
                stu_nonres = ifelse((stu_internat==0 & univ_state==stu_state), 0, NA),
                stu_nonres = ifelse((stu_internat==0 & univ_state!=stu_state), 1, stu_nonres))
       
       lists_orders_zip_hs_df %>% count(stu_nonres, stu_internat)
       
       lists_orders_zip_hs_df %>% group_by(stu_country, stu_internat) %>% count() %>% print(n=200)
       lists_orders_zip_hs_df %>% group_by(stu_country, stu_nonres) %>% count() %>% print(n=200)
       
       # race
       lists_orders_zip_hs_df %>% filter(stu_nonres==1) %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       
            # Out of state students: 52% White, 31% Asian, 8% Latino, 2% Black, 5% multiracial
       
       lists_orders_zip_hs_df %>% filter(stu_nonres==0) %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       
            # In state students: 44% White, 11% Asian, 29% Latino, 8% Black, 4% multiracial
       
       #income 
       lists_orders_zip_hs_df %>% filter(stu_country=="united states" & is.na(zip_median_household_income)) %>% count()
       ...
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% group_by(stu_nonres) %>% 
         summarise (mean_inc = mean(zip_median_household_income, na.rm=T)) # KS: add count of obs as well as %
       
            # Out of state Med Inc= $117,853; In-state $85,350
       
       #COL 2: Regional/Research Univ by race/ethnicity, avg income
       lists_orders_zip_hs_df %>% count(univ_c15basic)
       
       #in-state versus out of state
       lists_orders_zip_hs_df %>% filter(univ_c15basic==15) %>% group_by(stu_nonres) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       ...
       lists_orders_zip_hs_df %>% filter(univ_c15basic!=15) %>% group_by(stu_nonres) %>% 
         ...
         mutate(pct = 100*(n / sum(n)))
            ...
            # Research: 54% out of state, 37% in-state, 8% international?
            # Regional: 2% out of state; 98% in-state, <1% international 
       
       # race
       lists_orders_zip_hs_df %>% filter(univ_c15basic==15) %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       
       lists_orders_zip_hs_df %>% filter(univ_c15basic==15 & stu_nonres==0) %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       ...
       lists_orders_zip_hs_df %>% filter(univ_c15basic!=15 & stu_nonres==1) %>% group_by(stu_race_cb) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n)))
       
       lists_orders_zip_hs_df %>% filter(univ_c15basic!=15 & stu_nonres==1) %>%  
         count(stu_state) %>% arrange(desc(n)) %>% print(n=100)
       
              # Research: 48% White, 26% Asian, 14% Latino; 4% Black  (overall)
              # Research: 49% White, 10% Asian, 25% Latino; 8% Black  (in-state)
              # Regional: 42% White, 12% Asian, 31% Latino, 8% Black 
       
       #income 
       lists_orders_zip_hs_df %>% count(univ_c15basic, univ_name)
       lists_orders_zip_hs_df %>% count(univ_state, univ_name)
       
       
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% group_by(univ_name) %>% 
         summarise (mean_inc = mean(zip_median_household_income, na.rm=T)) 
       
       lists_orders_zip_hs_df %>% filter(stu_country=="united states" ) %>% group_by(univ_name, stu_nonres) %>% 
         summarise (mean_inc = mean(zip_median_household_income, na.rm=T)) 
       
       lists_orders_zip_hs_df %>% filter(stu_country=="united states" & univ_c15basic!=15) %>%  group_by(stu_nonres) %>%
         summarise (mean_inc = mean(zip_median_household_income, na.rm=T)) 
       
       # Research Avg Med Inc= $105355; (overall); $85535; (in-state); $119,056 (out-of-state)
       # Regional Avg Med Inc= ~$84,998 (overall); 67,764 (in-state); $85,267 (out-of-state)
       
       
      # Map-- show cities/metros where prospects are purchased? urban vs rural?
      
       #overall, top states/markets =in-state
       lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% group_by(stu_state) %>% 
        summarise (n = n()) %>%
        mutate(pct = 100*(n / sum(n))) %>% arrange(-n) %>% print(n=200)
       
      lists_orders_zip_hs_df %>% filter(stu_country=="united states") %>% group_by(stu_geomarket) %>% 
         summarise (n = n()) %>%
         mutate(pct = 100*(n / sum(n))) %>% arrange(-n) %>% print(n=200)
       
      
      # for out of state student states/markets
      lists_orders_zip_hs_df %>% filter(stu_country=="united states" & stu_nonres==1) %>% group_by(stu_state) %>% 
        summarise (n = n()) %>%
        mutate(pct = 100*(n / sum(n))) %>% arrange(-n) %>% print(n=200) 
      
      lists_orders_zip_hs_df %>% filter(stu_country=="united states" & stu_nonres==1) %>% group_by(stu_geomarket) %>% 
        summarise (n = n()) %>%
        mutate(pct = 100*(n / sum(n))) %>% arrange(-n) %>% print(n=200)
