...
Segment filters come from the College Board’s “Segment Analysis Service” which merges demographic, geographic, and academic data on SAT test takers to create "geodemographic profiles" for college-bound students (The College Board, 2011, p. 3). These profiles are created at the neighborhood-level and at the school-level. The `r (orders_filters1 %>% filter(filters == 'segment'))$V1` orders using segment filters were made by three public research universities in the sample. Figures \@ref(tab:orders-cluster-en) and \@ref(tab:orders-cluster-hs) show which neighborhood and high school segments were used across these orders and the characteristics of these segments. 

For example, one university made 21 of the 45 orders using segment filters, and all 21 orders followed the same patterns of segments. These 21 orders filtered for 10 different neighborhood clusters (51, 53, 58, 60, 61, 63, 69, 70, 73, and 78). Using average characteristics of neighborhood clusters calculated and advertised by the College Board in Figure X, the filtered neighborhoods range from median household incomes between \$68,213 to \$134,400 and range from 19% to 87% non-white.  Orders also filtered by high school segment clusters 58, 63, 64, 65, 66, 68, 69, 70, 73, 75, and 79. Figure X shows these high schools range from average SAT mathematics scores between 390 to 606 and previous cohorts in these schools on average ranged from 32% to 74% in going out of state for college. 
