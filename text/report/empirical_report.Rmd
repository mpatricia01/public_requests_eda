...
The ten most commonly used combinations of filters, which account for over 80% of all orders, are presented in Table \@ref(tab:orders-filter-combos).<!-- Check: sum((df_0 %>% head(10))$n) / sum(df_0$n) --> More than half of all orders analyzed used a combination of high school graduation class, zip code, GPA, and PSAT and/or SAT scores to filter prospect lists. This includes `r (df_0 %>% filter(string == 'grad_class,zip,psat,gpa'))$n` orders (`r (df_0 %>% filter(string == 'grad_class,zip,psat,gpa'))$pct`%) that used PSAT scores, `r (df_0 %>% filter(string == 'grad_class,zip,sat,gpa'))$n` orders that used SAT scores (`r (df_0 %>% filter(string == 'grad_class,zip,sat,gpa'))$pct`%), and `r (df_0 %>% filter(string == 'grad_class,zip,sat,psat,gpa'))$n` orders that used both PSAT and SAT (`r (df_0 %>% filter(string == 'grad_class,zip,sat,psat,gpa'))$pct`%) in addition to graduation class, zip code, and GPA. Other orders used a similar pattern of filters except for using state geographic filters instead of zip code, including `r (df_0 %>% filter(string == 'grad_class,state,psat,gpa'))$n` orders using PSAT scores and another `r (df_0 %>% filter(string == 'grad_class,state,sat,gpa'))$n` orders using SAT scores. 
