...
Figure X shows how often filters were used across all orders. All 486 orders filtered by high school graduation class. The most frequently used filters include GPA (94%), PSAT (60%), SAT (56%), and zip code (57%). About two in every five orders also filtered by state. Only a subset of orders filtered by race/ethnicity (19%), academic rank (10%), gender (5%), segment (5%), AP score (5%), core based statistical area (3%), geomarket (3%), and international (3%).
...
Orders further filtered for prospects by adding in high school segment within neighborhood. For example, within census tracts assigned to neighborhood cluster 51, only schools assigned to high school cluster 65, 68, 70, and 79 were included. Other high school clusters used in combination with the 10 neighborhood clusters include 64, 58, 
