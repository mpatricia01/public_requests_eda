...
rq3 %>% 
  ...
  mutate(
    row_subj = recode_factor(
      ...
      'med_income' = 'Median household income (mean)',
      'pct_city' = '% City',
  ...
  arrange(row_subj) %>% 
  kable(booktabs = T, col.names = c('', 'All domestic', 'GPA', 'PSAT', 'SAT', 'HS rank', 'AP score', 'Zip code', 'State', 'Segment', 'CBSA', 'Race', 'Gender'), align = c('l', rep('r', 12))) %>% 
  add_header_above(c(' ' = 2, 'Academic' = 5, 'Geographic' = 4, 'Demographic' = 2), bold = T) %>% 
  group_rows('Location', 2, 3) %>%
  group_rows('Race/ethnicity', 4, 13) %>%
  group_rows('Gender', 14, 17) %>%
  group_rows('Income', 18, 18) %>%
