...
```{r, fig.height = 5.5}
create_asu_la_race_figure <- function(data_df, category, plot_title) {
  data_df %>% 
    filter(ord_type == category) %>% 
    mutate(
      stu_race_cb = if_else(is.na(stu_race_cb), 999, unclass(stu_race_cb)),
      race = recode_factor(
        stu_race_cb,
...
create_asu_la_income_figure <- function(data_df, category, plot_title) {
  data_df %>% 
    filter(ord_type == category) %>% 
    mutate(
      count_type = recode_factor(
    ...
    geom_bar(stat = 'identity', fill = color_palette[[2]]) + 
    geom_text(mapping = aes(label = str_c('$', round(income_2564 / 1000), 'K')), size = 2, hjust = -0.1) +
    scale_x_continuous(expand = expansion(mult = c(0, 0.04)), limits = c(0, 191000)) +
    scale_y_discrete(expand = expansion(mult = c(0.01, 0.1))) +
    xlab('') + ylab('') +
    ggtitle(str_c(plot_title, '\n')) +
    guides(fill = guide_legend(reverse = T)) +
...
grid.arrange(
  create_asu_la_income_figure(asu_la_income, 'psat_low', 'Low PSAT order (1110-1210)'),
  create_asu_la_race_figure(asu_la_race, 'psat_low', 'Low PSAT order (1110-1210)'),
  create_asu_la_income_figure(asu_la_income, 'psat_med', 'Medium PSAT order (1190-1260)'),
  create_asu_la_race_figure(asu_la_race, 'psat_med', 'Medium PSAT order (1190-1260)'),
  create_asu_la_income_figure(asu_la_income, 'psat_high', 'High PSAT order (1270-1520)'),
  create_asu_la_race_figure(asu_la_race, 'psat_high', 'High PSAT order (1270-1520)'),
  create_asu_la_income_figure(asu_la_income, 'sat_med', 'Medium PSAT order (1140-1260)'),
  create_asu_la_race_figure(asu_la_race, 'sat_med', 'Medium PSAT order (1140-1260)'),
  ncol = 2,
  widths = c(2, 3)
)
```
